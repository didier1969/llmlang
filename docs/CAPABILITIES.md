...
| Performance (RE-MESURÉE 2026-07-14 après l'`Int` exact, DEC-LLL-077) | Harnais `bench/cspeed/run.sh` (CPU utilisateur, min/5, `-C overflow-checks=on` des DEUX côtés). **Taxe par opération ~4-6×, constante** : `lcg` 100M arithmétique **0,79 s** vs **0,16 s** Rust i64 *à travail égal* (**4,9×**) vs **0,29 s** C (2,7×) · `fib(40)` **2,32 s** vs **0,65 s** Rust (**3,6×**) · `listsum` 0,40 s vs 0,07 s C (5,7×) · `map` 0,65 s vs 0,39 s C (**1,7× — seul régime épargné**, dominé par le BTreeMap). ⚠ **Piège de banc** : LLVM fusionne algébriquement 5 pas du LCG en 1 sur de l'`i64` brut (récurrence affine mod 2^31) → son binaire ne fait que 20M pas (0,03 s). llmlang OBTENAIT cette fusion avant DEC-077 — c'est de là que venait l'ancien « ~10× plus rapide que C ». Le boxage coûte donc DEUX fois : ~5× par opération, **et** les réécritures que l'optimiseur ne voit plus. Les DEUX anciennes revendications (« ≤5 % vs Rust sur fib(40) », « ~10× plus rapide que gcc -O2 C ») sont **RETIRÉES**. Sortie : **REQ-LLL-162** (déboxage guidé par la preuve, récupère les deux moitiés). | `bench/cspeed/run.sh` (harnais reproductible) |
