...
| Performance (RE-MESURÉE 2026-07-14 après l'`Int` exact, DEC-LLL-077) | Harnais `bench/cspeed/run.sh` (CPU utilisateur, min/5, `-C overflow-checks=on` des DEUX côtés — la posture que `lll build` livre) : `lcg` 100M arithmétique **0,82 s** vs **0,02 s** Rust i64 vs **0,26 s** C (**~41×** Rust, **3,2× plus LENT que C**) · `fib(40)` call-heavy **2,33 s** vs **0,68 s** Rust (**~3,4×**) · `listsum` 0,41 s vs 0,08 s C (~5,1×) · `map` 0,59 s vs 0,38 s C (**1,6× — le plus proche de C**). Les DEUX anciennes revendications (« ≤5 % vs Rust sur fib(40) », « ~10× plus rapide que gcc -O2 C sur le LCG ») sont **RETIRÉES** : elles dataient de `Int` = `i64` brut. Cause = le BOXAGE (16 o, non-`Copy`, drop glue), pas l'arithmétique. Sortie spécifiée : **REQ-LLL-162** (déboxage guidé par la preuve). | `bench/cspeed/run.sh` (harnais reproductible) |
