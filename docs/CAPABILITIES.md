...
| Performance (RE-MESURÉE 2026-07-14 après l'`Int` exact, DEC-LLL-077) | Harnais `bench/cspeed/run.sh` (CPU utilisateur, min/5) : `lcg` 100M arithmétique **0,88 s** vs **0,02 s** Rust i64 vs **0,27 s** C (**~44×** Rust, **3,3× plus LENT que C**) · `fib(40)` call-heavy **2,31 s** vs **0,41 s** Rust (**~5,6×**) · `listsum` 0,39 s vs 0,08 s C · `map` 0,47 s vs 0,35 s C (**reste compétitif C**, 1,3×). Les DEUX anciennes revendications (« ≤5 % vs Rust sur fib(40) », « ~10× plus rapide que gcc -O2 C sur le LCG ») sont **RETIRÉES** : elles dataient de `Int` = `i64` brut. Cause = le BOXAGE (16 o, non-`Copy`, drop glue), pas l'arithmétique. Sortie spécifiée : **REQ-LLL-162** (déboxage guidé par la preuve). | `bench/cspeed/run.sh` (harnais reproductible) |
