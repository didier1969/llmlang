...
| Performance — **exécution SPÉCULATIVE** (REQ-LLL-162 : jumeau `i64` brut + repli exact ; sain car le langage est PUR) | Harnais `bench/cspeed/run.sh` (CPU utilisateur, min/5, `-C overflow-checks=on` des deux côtés) : `lcg` 100M arithmétique **0,03 s** vs 0,02 s Rust i64 vs 0,28 s C → **~10× plus RAPIDE que gcc -O2 C** (LLVM refusionne 5 pas du LCG en 1 sur de l'i64 brut ; gcc n'y arrive pas) · `fib(40)` **0,96 s** vs 0,68 s Rust (**1,4×**, contre 3,6× en boxé) · `map` 0,52 s vs 0,36 s C. ⚠ **Reste honnêtement lent** : `listsum` **0,41 s vs 0,07 s C (~5×)** — les ÉLÉMENTS d'une `List[Int]` restent boxés (16 o/cellule) et la spéculation est dirigée par la signature. Prochaine frontière. | `bench/cspeed/run.sh` · `tests/integration/fastpath.rs` |
