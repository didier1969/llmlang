...
fn export_evidence(file: &str) -> Result<String, String> {
    ...
    let out = serde_json::json!({
        ...
        "vcgen_version": vc::VCGEN_VERSION,
        // REQ-LLL-155 tranche 2c : la version du SOLVEUR fait partie de l'identité de preuve (le
        // cache la binde). Une preuve est le produit JOINT de la logique vcgen ET du solveur qui
        // l'a déchargée → l'évidence/attestation la porte (base du tuple {def_hash, proof_hash,
        // Z3-version} que la distribution cross-projet suppose).
        "z3_version": vc::z3_version(),
        "evidence": evidence,
    });
    serde_json::to_string_pretty(&out).map_err(|e| e.to_string())
}
